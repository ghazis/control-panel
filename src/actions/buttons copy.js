...
export function decrementTemp(new_temp) {
    return (dispatch, getState) => {
        ...
        fetch('http://73.209.181.138:7000/set_temp?temp=' + new_temp);
    };
}
...
export function runAutoMode(current_temp, desired_temp, name, cmd) {
    ...
    return (dispatch, getState) => {
        ...
        if (cmd == '_on') {
            ...
            if (auto_type == 'AC') {
                fetch('http://73.209.181.138:7000/cmd?cmd=ac_off');
            } else {
                fetch('http://73.209.181.138:7000/cmd?cmd=heat_off');
            }
        }
...
export function AutoModeOn() {
    return (dispatch, getState) => {
        ...
        if (current_temp > desired_temp) {
            fetch('http://73.209.181.138:7000/cmd?cmd=ac_on');
        } else if (current_temp < desired_temp) {
            fetch('http://73.209.181.138:7000/cmd?cmd=heat_on');
